...
addr_range_start=$((0xfff884600000))
...
while [ $addr_range_loop -lt $addr_range_end ]; do
	...
	while [ $inner_loop -lt $parallel_idx ] && [ $addr_range_loop -lt $addr_range_end ]; do
		((image_counter++))
		damo report heats -i ${workload}_native.data --resol 1000 1 --address_range ${addr_range_loop} ${addr_range_next} --abs_addr > ../files/hashjoin/${workload}_${addr_range_loop}.txt &
		((inner_loop++))
