...
static struct folio *alloc_anon_folio(struct vm_fault *vmf)
{
	...
	pr_info("alloc_anon_folio [ENTRY]\n");
	pr_info("\tOrders available for this fault: %lu\n", orders);
	...
	pr_info("\tHighest order of available orders: %d\n", order);
#ifdef CONFIG_PFTRACE
        cycles = get_cycles();
#endif /* CONFIG_PFTRACE */
	mm_action.action = MM_ACTION_NONE;
	mm_action.address = vmf->real_address;
	mm_action.hugepage_orders = orders;
	mm_estimate_changes(&mm_action, &mm_cost_delta);
	pr_info("\tOrders after eBPF: %lu\n", mm_action.hugepage_orders);
	should_do = mm_decide(&mm_cost_delta);

	if (mm_action.action == MM_ACTION_EBPF_HOOK) {
		pr_info("hugepage ebpf program hooked\n");
		orders = mm_action.hugepage_orders;
		order = highest_order(orders);
	}
#ifdef CONFIG_PFTRACE
	if (tracepoint_enabled(ebpfoverhead)) {
		do_trace_ebpfoverhead(get_cycles() - cycles);
	}
	cycles = get_cycles();
#endif /* CONFIG_PFTRACE */


	/* TODO: Here I should return in bit type which orders of 
	 * the available ones are beneficial enough to promote to.
	 * 
	 * Then a bitwise or will determine if we should try to 
	 * upgrade to that particualar huge page size.
	 */
	while (orders) {
	...
	while (orders) {
		...
		if (folio) {
			pr_info("Promoting to huge page of order: %d\n", order);
			...
			folio_throttle_swaprate(folio, gfp);
                #ifdef CONFIG_PFTRACE
                        if (tracepoint_enabled(allocmthppage)) {
                                do_trace_allocmthppage(get_cycles() - cycles);
                        }
                        cycles = get_cycles();
                #endif /* CONFIG_PFTRACE */
...
retry_pud:
	...
	if (pmd_none(*vmf.pmd) &&
		...
		mm_action.action = MM_ACTION_PMD;
		mm_action.address = address;
		mm_action.hugepage_orders = ( 1UL << 8 );
		mm_estimate_changes(&mm_action, &mm_cost_delta);

		if (mm_action.hugepage_orders)
		{
			ret = create_huge_pmd(&vmf);
			if (!(ret & VM_FAULT_FALLBACK)) {
				pr_info("Created a huge pmd page.\n");
				return ret;
			}
		}
	} else {
